...
// Models (Updated paths for absolute root)
const User = require('../models/User');
const Product = require('../models/Product');
const Order = require('../models/Order');
const Cart = require('../models/Cart');

// Services (Updated paths for absolute root)
const cloudinary = require('../config_server/cloudinary');
...
app.post('/api/products', async (req, res) => {
  try {
    ...
    try {
      console.log("DEBUG: Starting Notification Bridge check (Root API)...");
...
app.get('/api/db-status', async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ 
      success: true, 
      message: 'Database Connected Successfully! (Flattened API)', 
...
app.get('/', (req, res) => {
  res.send('Wafa Hardware Root API is running!');
});
